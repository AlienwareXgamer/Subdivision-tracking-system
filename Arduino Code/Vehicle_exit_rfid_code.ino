...
void setup() {
  ...
  // Initialize SPI bus with a faster clock speed for shorter cable length (supporting up to 4.5 meters)
  SPI.begin();
  SPI.setClockDivider(SPI_CLOCK_DIV16);  // Set SPI speed to 1 MHz for faster communication

  // Initialize MFRC522 RFID module
  mfrc522.PCD_Init();
  Serial.println(F("Vehicle Exit RFID system initialized. Scan a card..."));
...
void updateLogs(String status) {
  if (status == "accepted") {
    ...
    logToFirebase("latest_exit_log", "accepted", "Exit", "Resident found");
  } else if (status == "denied") {
    // Log to denied_entry_log for denied entries
    logToFirebase("denied_entry_log", "denied", "Exit", "Resident not found or UID not assigned");
  }
